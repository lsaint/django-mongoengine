...
class DictField(forms.Field):
    ...
    def __init__(
        self, max_depth=None, flags=None, sub_attrs=None, attrs=None, *args, **kwargs
    ):
        ...
        for k, v in value.items():
            ...
            if k in self.invalid_keys:
                raise ValidationError(
                    self.error_messages["invalid_key"] % self.invalid_keys
                )
            ...
            for u in self.illegal_characters:
                if u in k:
                    raise ValidationError(
                        self.error_messages["illegal"] % self.illegal_characters
                    )

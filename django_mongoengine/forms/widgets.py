...
from vendor.django_mongoengine.utils import OrderedDict
...
class Dictionary(MultiWidget):
    ...
    def render(self, name, value, attrs=None):
        ...
        for i, widget in enumerate(self.widgets):
            ...
            output.append(
                widget.render("%s_%s_%s" % (name, i, suffix), widget_value, final_attrs)
            )
    ...
    def value_from_datadict(self, data, files, name):
        ...
        for data_key in data_keys:
            ...
            if match is not None:
                self.widgets.append(
                    self.pair(sub_attrs=self.sub_attrs, attrs=self.attrs)
                )
    ...
    def update_widgets(self, keys, erase=False):
        ...
        for k in keys:
            if isinstance(k[1], dict):
                ...
                self.widgets.append(
                    self.pair(
                        sub_attrs=self.sub_attrs, key_value=k[1], attrs=self.attrs
                    )
                )
...
class SubDictionary(Pair):
    ...
    def __init__(self, sub_attrs, schema=None, **kwargs):
        if schema is None:
            schema = {"key": "value"}
        super(SubDictionary, self).__init__(
            schema=schema, sub_attrs=sub_attrs, **kwargs
        )
...
class EmbeddedFieldWidget(MultiWidget):
    ...
    def __init__(self, fields, attrs=None):
        self.fields = fields
        super(EmbeddedFieldWidget, self).__init__(
            [f.widget for f in self.fields.values()], attrs
        )

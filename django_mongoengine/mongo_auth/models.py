...
from vendor.django_mongoengine import document
from vendor.django_mongoengine import fields
from vendor.django_mongoengine.queryset import QuerySetManager
...
ContentTypeManager = type(
    "ContentTypeManager",
    (QuerySetManager,),
    dict(ContentTypeManager.__dict__, __init__=ct_init),
)
...
class AbstractUser(BaseUser, document.Document):
    ...
    last_login = fields.DateTimeField(
        default=timezone.now, verbose_name=_("last login")
    )
    ...
    meta = {
        "abstract": True,
        "indexes": [{"fields": ["username"], "unique": True, "sparse": True}],
    }

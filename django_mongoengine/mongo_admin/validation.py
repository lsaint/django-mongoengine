...
def _validate(cls, model):
            ...
            if not callable(field):
                if not hasattr(cls, field):
                    if not hasattr(model, field):
                        ...
                        f = fetch_attr(
                            cls, model, opts, "list_display[%d]" % idx, field
                        )
    ...
    if hasattr(cls, "list_per_page") and not isinstance(cls.list_per_page, int):
        raise ImproperlyConfigured(
            "'%s.list_per_page' should be a integer." % cls.__name__
        )
    ...
    if cls.date_hierarchy:
        ...
        if not isinstance(f, (models.DateField, models.DateTimeField)):
            raise ImproperlyConfigured(
                "'%s.date_hierarchy is "
                "neither an instance of DateField nor DateTimeField." % cls.__name__
            )
        ...
        for idx, field in enumerate(cls.ordering):
            if field == "?" and len(cls.ordering) != 1:
                raise ImproperlyConfigured(
                    ...
                    "well. Please either remove '?' or the other fields." % cls.__name__
                )
            if field == "?":
                continue
            if field.startswith("-"):
...
def validate_inline(cls, parent, parent_model):
    ...
    if not issubclass(cls, EmbeddedDocumentAdmin):
        fk = _get_foreign_key(
            parent_model, cls.model, fk_name=cls.fk_name, can_fail=True
        )
    else:
        fk = None
    ...
    if max_num is not None and not isinstance(max_num, int):
        raise ImproperlyConfigured(
            "'%s.max_num' should be an integer or None (default)." % cls.__name__
        )
        ...
        for idx, fieldset in enumerate(cls.fieldsets):
            ...
            if "fields" not in fieldset[1]:
                raise ImproperlyConfigured(
                    ...
                    "%s.fieldsets[%d][1] field options dict." % (cls.__name__, idx)
                )
            for fields in fieldset[1]["fields"]:
                ...
                for field in fields:
                    if field in cls.readonly_fields:
                        ...
                        continue
                    check_formfield(
                        cls, model, opts, "fieldsets[%d][1]['fields']" % idx, field
                    )
                    try:
                        f = opts.get_field(field)
                        if (
...
def check_isdict(cls, label, obj):
    if not isinstance(obj, dict):
        raise ImproperlyConfigured(
            "'%s.%s' must be a dictionary." % (cls.__name__, label)
        )
...
def get_field(cls, model, opts, label, field):
    try:
        ...
        raise ImproperlyConfigured(
            "'%s.%s' refers to field '%s' that is missing from model '%s'."
    ...
    if getattr(cls.form, "base_fields", None):
        ...
        try:
            ...
            if hasattr(model, field) and isinstance(
                getattr(model, field), EmbeddedDocumentField
            ):
                return
            raise ImproperlyConfigured(
                "'%s.%s' refers to field '%s' that "
...
def fetch_attr(cls, model, opts, label, field):
    try:
        ...
        raise ImproperlyConfigured(
            "'%s.%s' refers to '%s' that is neither a field, method or property of model '%s'."
            % (cls.__name__, label, field, model.__name__)
        )
